        ...
        <section className="w-full pl-2">
            ...
            <div ref={ref} className={`w-full grid grid-cols-2 lg:grid-cols-4 gap-2 mt-6 ${style["service-container"]}`}>
                {ourServices.map((data) => (
                    <div key={data.service} className={`w-full service-box overflow-hidden relative opacity-0 rounded-[8px] ${style["service-card"]} h-[22rem] lg:h-[25rem] pb-[8rem] lg:pb-0`}>
                        ...
                        <p className="px-2 text-[0.7rem] lg:text-[0.9rem] mt-2">{data.description}</p>
